...
import { HttpType } from "@/views/vip_replenish_recharge/setting";
import { MessageBox } from "element-ui";
import IVipReplenishRechargeProxy from "./IVipReplenishRechargeProxy";

export default class VipReplenishRechargeProxy extends AbstractProxy implements IVipReplenishRechargeProxy {
    static NAME = "VipReplenishRechargeProxy";

    /**进入页面时调用 */
    enter() {
        this.sendNotification(HttpType.admin_plat_user_table_columns);
        this.sendNotification(HttpType.admin_vip_replenish_recharge_table_columns);
    }
    ...
    onTopup() {
        ...
        this.sendNotification(HttpType.admin_vip_replenish_recharge_recharge, objectRemoveNull(this.bodyData.form));
    }
    /**重置页面表单 */
    resetForm() {
