import AbstractMediator from "@/core/abstract/AbstractMediator";
import { SuccessMessage } from "@/core/global/Constant";
import { IEventDispatcher } from "@/core/IEventDispatcher";
import { EventType, HttpType } from "@/views/admin_role/setting";
import { Message } from "element-ui";
import AdminRoleProxy from "../proxy/AdminRoleProxy";

interface IAdminRole extends IEventDispatcher {
    
}

export default class AdminRoleMediator extends AbstractMediator {
    private myProxy: AdminRoleProxy = <any>this.getProxy(AdminRoleProxy);

    onRegister() {
        this.myProxy.enter();
    }

    onRemove() {
        this.myProxy.leave();
    }

    protected initViewData() {
        const myView: IAdminRole = this.viewComponent;
    }

    listNotificationInterests(): string[] {
        return [
            EventType.admin_admin_role_table_columns,
            EventType.admin_admin_role_index,
            EventType.admin_admin_role_show,
            EventType.admin_admin_role_store,
            EventType.admin_admin_role_update,
            EventType.admin_admin_user_mine,

        ];
    }

    handleNotification(notification: puremvc.INotification) {
        const myProxy: AdminRoleProxy = <any>this.facade.retrieveProxy(AdminRoleProxy.NAME);
        const myView: IAdminRole = this.viewComponent;
        const body = notification.getBody();
        switch (notification.getName()) {
            case EventType.admin_admin_role_table_columns:
                myProxy.setTableColumns(body);
                break;
            case EventType.admin_admin_role_index:
                myProxy.setTableData(body);
                break;
            case EventType.admin_admin_role_show:
                myProxy.setDetail(body);
                break;
            case EventType.admin_admin_role_store:
                Message.success(SuccessMessage.create);
                myProxy.hideDialog();
                myProxy.listQuery.page_count = 1;
                myProxy.onQuery();
                break;
            case EventType.admin_admin_role_update:
                Message.success(SuccessMessage.update);
                myProxy.hideDialog();
                myProxy.hidedialogSetting();
                myProxy.onQuery();
                break;
            case EventType.admin_admin_user_mine:
                myProxy.setPermission(body.permission);
                break;

        }
    }
}
