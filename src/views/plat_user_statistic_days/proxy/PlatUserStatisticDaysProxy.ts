...
import { formCompared, objectRemoveNull, dateFormat, getTodayOffset } from "@/core/global/Functions";
import { HttpType } from "@/views/plat_user_statistic_days/setting";
import { MessageBox } from "element-ui";
import IPlatUserStatisticDaysProxy from "./IPlatUserStatisticDaysProxy";
import GlobalEventType from "@/core/global/GlobalEventType";

export default class PlatUserStatisticDaysProxy extends AbstractProxy implements IPlatUserStatisticDaysProxy {
    static NAME = "PlatUserStatisticDaysProxy";

    /**进入页面时调用 */
    enter() {
        this.sendNotification(HttpType.admin_plat_user_statistic_days_table_columns);
    }
    ...
    showDialog(status: string, obj?: any) {
        ...
        if (status == DialogStatus.update) {
            ...
            data.total_recharge = data.total_recharge - 0;
            data.total_water = data.total_water - 0;
            data.total_win = data.total_win - 0;
    ...
    /**查询 */
    onQuery() {
        this.sendNotification(HttpType.admin_plat_user_statistic_days_index, objectRemoveNull(this.listQuery));
    }
    ...
    onAdd() {
        ...
        this.sendNotification(HttpType.admin_plat_user_statistic_days_store, objectRemoveNull(formCopy));
    }
    ...
    onUpdate() {
        ...
        this.sendNotification(HttpType.admin_plat_user_statistic_days_update, formCopy);
    }
