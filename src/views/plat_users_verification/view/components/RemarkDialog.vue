    ...
    <el-dialog :title="LangUtil('提示')" :visible.sync="myProxy.remarkDialogData.bShow" width="550px">
        <div v-loading="net_status.loading">
            <el-form
                ...
                :model="form"
            >
                <p class="desc" v-if="myProxy.remarkDialogData.form.desc">
                    <i class="el-icon-warning"></i><span>{{ myProxy.remarkDialogData.form.desc }}</span>
                </p>
...
export default class RemarkDialog extends AbstractView {
    ...
    // proxy
    myProxy: PlatUsersVerificationProxy = this.getProxy(PlatUsersVerificationProxy);
    // proxy property
    ...
    handleUpdate() {
        (this.$refs["form"] as Vue & { validate: (cb: any) => void }).validate((valid: boolean) => {
            if (valid) {
                ...
                // } else {
                if (this.myProxy.isDandlerDelete.status) {
                    this.myProxy.remarkDialogData.bShow = false;
                    this.myProxy.showTip(this.myProxy.isDandlerDelete.data, false);
                    this.myProxy.isDandlerDelete.status = false;
                } else {
