<template>
    <el-dialog
        :title="`${textMap[status]}` + $t('plat_users_vip_model.title')"
    ...
    >
        <el-form ref="form" :rules="rules" :model="form" label-width="150px" v-loading="net_status.loading">
            <el-form-item size="mini" :label="$t('plat_users_vip_model.name')" prop="name">
                <el-input v-model="form.name"></el-input>
            </el-form-item>
            <el-form-item size="mini" :label="$t('plat_users_vip_model.desc')" prop="desc">
                ...
                <div class="add_level">
                    ...
                    <el-checkbox-group v-model="form.types">
                        <el-checkbox :label="1" name="type" @change="handlerTypesChange">{{
                            $t("plat_vip.effectiveFlow")
                        }}</el-checkbox>
                        <el-checkbox :label="2" name="type" @change="handlerTypesChange">{{
                            $t("plat_vip.totalRecharge")
                        }}</el-checkbox>
                    </el-checkbox-group>
                    <span v-show="form.types.length == 0" class="red"> {{ $t("plat_users_vip_model.atLeast") }}</span>
                </div>
            </el-form-item>
            <div>
                {{ $t("common.topLevel") }}<span class="red">{{ form.vip_config.length }}</span
                >{{ $t("common.level") }}
            </div>
                ...
                <el-col :span="20">
                    <div class="inner_border">
                        <div class="level_setting">
                            <div class="level">{{ $t("common.level") }} {{ index + 1 }}</div>
                            <div>
                                <el-form-item
                                    v-if="form.types.indexOf(1) != -1"
                                    size="mini"
                                    :label="$t('plat_vip.effectiveFlow')"
                                >
                                    <el-input
                            ...
                            <div>
                                <el-form-item
                                    ...
                                    size="mini"
                                    :label="$t('plat_vip.totalRecharge')"
                                >
                                    <el-input
                        ...
                        <div class="gap_line">{{ $t("plat_users_vip_model.backWater") }}</div>
            ...
            <el-form-item class="dialog-footer">
                ...
                <el-button type="primary" size="mini" @click="isStatusUpdate && checkUnique(unique.plat_users_vip_model_update) ? handleUpdate() : handleAdd()">{{
                    $t("common.save")
                }}</el-button>
            </el-form-item>
        </el-form>
...
export default class PlatUsersVipModelDialog extends AbstractView {
    ...
    private handlerAddLevel() {
        this.myProxy.addLevel();
    }

    private handlerDeleteLeve(index: number) {
